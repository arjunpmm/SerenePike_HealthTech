...
nn <- neuralnet(RESP~ PULSE + Age  + Gender +SpO2, datatrain, hidden=4, threshold=0.01, linear.output=FALSE)
plot(nn)
nn$result.matrix
...
pred=gbm(RESP ~ HR + PULSE + Age  + SpO2 + Gender,data = datatrain,distribution = "gaussian",n.trees = 10000,
